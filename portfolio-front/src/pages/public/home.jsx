...
/* =====
   Utils : garantit une URL absolue pour les médias (évite les doubles préfixes)
===== */
const toAbsoluteUrl = (path) => {
  if (!path) return null;
  if (/^https?:\/\//i.test(path)) return path; // déjà absolue
  const cleaned = String(path).replace(/^\/+/, ""); // évite // au début
  if (typeof buildUploadUrl === "function") {
    return buildUploadUrl(cleaned);
  }
...
export default function Home() {
  ...
  const currentExp =
    EXPERIENCES.find((e) => e.key === activeExp) || EXPERIENCES[0];
    ...
    <>
      ...
      <section className="section s-home">
        <div className="container grid">
          <div className="grid" style={{ alignContent: "center", gap: "var(--space-6)" }}>
          ...
          <div className="surface card card-padding" style={{ display: "grid", placeItems: "center" }}>
            <img src="/assets/hero.svg" alt="Logo A-PLCQ" width={140} height={140} />
          </div>
        </div>
      </section>
      ...
      <section className="section s-about">
        <div className="container grid grid-2">
          <div className="card surface card-padding" style={{ minHeight: 260 }}>
            <div
              aria-hidden
              style={{
                width: "100%",
                ...
                background:
                  "linear-gradient(135deg, rgba(152,109,255,.15), transparent 60%), url('/assets/img/hero.jpg') center/cover no-repeat",
          ...
          <div className="grid grid-2 mt-6">
            ...
            <div className="card surface card-padding">
              <div className="grid gap-4">
                ...
                <h5 className="companyExperience">{currentExp.titre}</h5>
      ...
      {/* SKILLS + description au survol */}
      <section className="section s-skills">
        <div className="container">
          <h2>Connaissances <span className="primary">.</span></h2>
          <p className="muted mt-2">* Survolez une carte pour voir la description *</p>
          ...
          <div className="grid grid-3 mt-6">
            {SKILLS.map((s) => (
              <article
                ...
                style={{ display: "grid", placeItems: "center", minHeight: 120, cursor: "default" }}
...
/* ---------- Carte projet : version robuste (sélection image + fallback) ---------- */
function ProjectCard({ projet }) {
  ...
  // 1) Sélectionne le 1er média image pertinent (type "image/*" ou "image")
  const firstImage =
    (media || []).find(
      (m) =>
  ...
  // 2) Construit une URL absolue et propre
  const img = firstImage?.chemin ? toAbsoluteUrl(firstImage.chemin) : null;

  // 3) Log utile en dev (désactive-le en prod)
  // console.log("MEDIA URL =>", firstImage?.chemin, "→", img);
  ...
  return (
    <div className="card surface card-hover" style={{ overflow: "hidden" }}>
      {img ? (
        <div className="thumb">
          <img
            ...
            style={{ transition: "transform .5s var(--ease)" }}
      ...
      <div className="card-padding" style={{ display: "grid", gap: "var(--space-4)" }}>
        <div>
          <h3 style={{ marginBottom: "6px" }}>{titre}</h3>
          {extrait && (
        ...
        <div className="field-row">
          <Link to={`/projets/${slug}`} className="btn btn-ghost">
            Voir le projet
          </Link>
          {github && (
            <a
